...
Each chunk is passed to AWS Bedrock Titan Embed v2, which returns a 1,024-dimension vector representing the semantic meaning of that text. Only changed documents get re-embedded — unchanged chunks already in ChromaDB are untouched. Three reasons Titan Embed v2 specifically: Credential continuity. AWS credentials are already in the project for S3. Titan Embed is Amazon's first-party model so no additional setup is required. You also get vector quality at reasonable size so dimensions are large enough to capture semantic nuance in technical documentation.
