/*
  ...
  Again according to MDN, once the prpmise is settled - means if the state is either returned or rejected, we have three methods .then(), .catch() and .finally()
  ...
  But what happens if the Promise fails. We then use the reject() method to capture the error and pass it into the '.catch()' method. 
  Let's modify our snippet to understand it further.
  ...
  Let's try to encapsulate all the learning in a simple example. 
  ...
  Promises work in an async manner. It means even if the 'resolve' function is written first, but since it has a longer timeout, the function gets executed
  after the second function. 
  
  
 Once the promise is fulfilled, you can take further action on the returned value by using the .then() method. Again according to MDN, 
 
 Creating a promise with the new Promise constructor. The new Promise constructor returns a promise object
...
the firstPromise below will be resolved after 5 seconds. we then use the promise.then() method to pass the value of the firstPromise after 
it is resolved and pass it as a parameter in the callback function. 

Creating a promise a Promise constructor and passing two parameters: resolve and reject in the callback function. Below we create a 'firstPromise' promise and 
then use the setTimeout method to do something when the promise is resolved
...
Attaching  a function that should run when the Promise resolves using the promise.then() method
...
firstPromise.then(function(msg){
 ...
 Let's use the knowledge in a live project. We learn how to create a Promise and fetch results from an API. Accessing external API is best 
 done using the Promise method. This is because we are not sure when will the API return back the results. We also need to consider the aspect 
 of the API rejecting our request, plus network result. Combining all these requests is best done using the fetch() method. The fetch() method takes a url as 
 a parameter. The URL is typically an API endpoint. There are various APIs available for testing. One such is the jsonplaceholder API.
  
  On accessing the endpoint, the API returns a response. We store the response as a parameter in an anonymous callback function. The response 
  is returned as a readable stream. This response is not of any value. 
...
fetch('https://jsonplaceholder.typicode.com/posts').then(function (response) {
 ...
 To make the response readable, we attach a then() method that reads the response
 and stores it in a variable called data. The data object is returned either as a JSON response or as text.
 
  //Returns a JSON response that can be used
...
The catch() method: The catch() method is responsible for catching any errors that the API returns. We again use an anonymous callback function and pass the error 
retrieved by the catch method into a variable.
  
}).catch(function(error){
    console.warn(error);

 I have created a page that combines the above methods and fetches response from 'The Stoic API', as well as 'The BhagavadGita API'. My next project will be to use the HAPI API to create a library of books
 ...
 console.log(favMovies); //(5) ['interstellar', 'the dark knight' , 'the prestige', 'oppenheimer', ''memento']
